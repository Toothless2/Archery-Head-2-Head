...
class SecondRoundScoreInput(val parent : MainActivity) : Fragment(), IScoreInput {
    ...
    private fun assignFunctionsToButtons() {
        p1end1.setOnClickListener {
            EventBus.keyboardEvent(KeyboardEvent(this, 1, 4))
        }

        p1end2.setOnClickListener {
            if (GameManager.completedEnds() >= 4)
                EventBus.keyboardEvent(KeyboardEvent(this, 1, 5))
        }

        if (!GameManager.isAiGame) {
            p2end1.setOnClickListener {
                EventBus.keyboardEvent(KeyboardEvent(this, 2, 4))
            }

            p2end2.setOnClickListener {
                if (GameManager.completedEnds() >= 4)
                    EventBus.keyboardEvent(KeyboardEvent(this, 2, 5))
            }
        }
    }
